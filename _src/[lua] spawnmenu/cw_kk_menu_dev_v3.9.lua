...
local function reloadAimPos()
	...
	local wepKey = wep:GetClass()
	
	if not sightName then 
		sightName = {["name"] = "ironsights"}
	end
		
	local attKey = sightName.name
	
	local AimPos = backup[wepKey][attKey].pos
	local AimAng = backup[wepKey][attKey].ang
...
concommand.Add("cw_kk_aim_reload", reloadAimPos)
...
local function KKSCK_Think()
	...
	local wepChanged = wep != last_wep
	
	if not wep.CW20Weapon then
		labels.sight:SetText("No CW2 weapon equiped.")
		if not wepChanged then 
	...
	if att != last_att then
		...
		local attKey = att.name
		
		if not backup[wepKey] then
			backup[wepKey] = {}
		end
		
		if not backup[wepKey][attKey] then
			backup[wepKey][attKey] = {
				pre = prefix,
				pos = Vector(math.Round(POS.x,4), math.Round(POS.y,4), math.Round(POS.z,4)),
				ang = Vector(math.Round(ANG.x,4), math.Round(ANG.y,4), math.Round(ANG.z,4)),
			}
		end
	end
...
local function CW2_ClientsidePanelDev(panel)
	...
	panel:AddControl("Button", {Label = "Reload", Command = "cw_kk_aim_reload"}):DockMargin(8, 0, 8, 8)
...
local function CW_KK_PopulateToolMenuDev()
	spawnmenu.AddToolMenuOption("Utilities", "CW 2.0 SWEPs", "Client (DEV v3.9)", "Client (DEV v3.9)", "", "", CW2_ClientsidePanelDev)
end

hook.Add( "PopulateToolMenu", "CW_KK_PopulateToolMenuDev_39", CW_KK_PopulateToolMenuDev )
