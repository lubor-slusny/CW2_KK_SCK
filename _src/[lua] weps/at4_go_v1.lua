...
if CLIENT then
	local c = Color(0,0,255,0)
	
	SWEP._generator_1337 = {}
	-- SWEP._generator_1337.frames = {1337,1338,13370,13380,133700,133800}
	...
	function SWEP:updateOtherParts()
		...
		if self.Sequence == self.Animations.base_pickup and cyc < 0.99 then
			...
			-- c.a = p * 255 
			-- self.AttachmentModelsVM.kk_counter_back.ent:SetColor(c)
			-- self.AttachmentModelsVM.kk_counter_back.ent:SetRenderMode(RENDERMODE_TRANSALPHA)
	...
	function SWEP:drawCounter(num, id)
		...
		if num then
			...
			for i = 1,6 do
				CustomizableWeaponry_KK.stattrack.setDigit2(i, raw[i])
				-- CustomizableWeaponry_KK.stattrack.setDigit(i, raw % 10)
				-- raw = math.Truncate(raw / 10)
			end
